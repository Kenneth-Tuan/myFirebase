...
exports.generateAuthUrl = onRequest({
  ...
  try {
    ...
    // 生成授權 URL
    const authUrl = tokenService.generateAuthUrl(state);
    ...
    logger.error("❌ 生成授權 URL 失敗:", error);
    res.status(500).json({
      success: false,
      error: error.message,
      oauthFlow: "web_server",
...
exports.handleOAuthCallback = onRequest({
  ...
  try {
    ...
    const tokenService = new TokenService();
    const result = await tokenService.handleAuthorizationCode(code, state);
    ...
    logger.error("❌ OAuth 回調處理失敗:", error);
    res.status(500).json({
      success: false,
      error: error.message,
      oauthFlow: "web_server",
      details: "請檢查授權碼是否正確且未過期",
    });
  }
});
...
exports.checkAuthStatus = onRequest({
  ...
  try {
    ...
    const needsReauth = await tokenService.needsReauthorization();
    ...
    if (needsReauth) {
      ...
      response.recommendations.push("未找到 token，請進行初始 OAuth 2.0 授權");
    }
      ...
      steps: [
        {
          ...
          endpoint: "/oauth/auth",
          method: "GET",
          parameters: ["client_id", "redirect_uri", "scope", "state", "response_type=code"],
        },
        {
          step: 2,
        ...
        },
        {
          ...
          security: ["Authorization code expires in 10 minutes", "Code can only be used once"],
        },
        {
          step: 4,
          name: "Token Exchange",
        ...
        },
        {
          ...
          security: ["Access tokens expire", "Use refresh tokens to get new access tokens"],
        },
      ],
      securityFeatures: [
